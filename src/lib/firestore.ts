...
export const setDoc: typeof firestore.setDoc = (async (reference: any, data: AnyRecord, options?: any) => {
  await executeWrite(
    () => options ? firestore.setDoc(reference, data, options) : firestore.setDoc(reference, data),
...
export const updateDoc: typeof firestore.updateDoc = (async (reference: any, dataOrField: any, ...moreFieldsAndValues: any[]) => {
  const data = typeof dataOrField === 'object' && dataOrField !== null ? dataOrField : {};
  await executeWrite(
    () => (firestore.updateDoc as any)(reference, dataOrField, ...moreFieldsAndValues),
...
export const writeBatch: typeof firestore.writeBatch = ((database: any) => {
  ...
  const facade: AnyRecord = {
    set(reference: any, data: AnyRecord, options?: any) {
      ...
      if (options) underlying.set(reference, data, options);
      else underlying.set(reference, data);
      return facade;
    },
    update(reference: any, dataOrField: any, ...moreFieldsAndValues: any[]) {
      ...
      (underlying.update as any)(reference, dataOrField, ...moreFieldsAndValues);
      return facade;
    },
    delete(reference: any) {
      activities.push({ action: 'deleted', ...referenceDetails(reference) });
      underlying.delete(reference);
      return facade;
    },
    async commit() {
      await ensureLanWriteAccess();
