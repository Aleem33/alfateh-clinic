...
import { db, registerSecondaryUser, usernameToEmail } from '../../firebase';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const cred = await registerSecondaryUser(form.username.trim(), form.password);
        await setDoc(doc(db, 'users', cred.user.uid), {
          name: form.name,
          username: form.username.trim().toLowerCase(),
          email: usernameToEmail(form.username.trim()),
          role: form.role,
          app: 'pos',
              ...
              <tr key={u.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-1">
                    ...
                    <button onClick={() => handleDelete(u.id, u.name)} className="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Delete">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              {!editingId && (
                <>
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">No spaces — use dots instead (e.g. john.doe)</p>
                  </div>
                  <div>
              ...
              {editingId && (
                <p className="text-xs text-amber-600 bg-amber-50 rounded-lg p-2">
                  Note: Username and password can only be changed by the user themselves in Settings.
                </p>
              )}
